...
.DEFAULT_GOAL := help
.PHONY: clean kodi pandoc3 pdf2djvu janet antiword zotero ditaa firestarter gmaptool tmx2lua hunspell-uk luajit ziglang languagetool gnuplot visidata sdcv ttyplot bandwhich bb gpxsee fio tailscale iosevka metabase mu kitty tiled libresprite
...
target/packages/p/pandoc3/pandoc3-3.5-3-1-x86_64.eopkg: target/Taskfile.yml
...
pandoc3: target/packages/p/pandoc3/pandoc3-3.5-3-1-x86_64.eopkg ## build pandoc3, print path to new eopkg file
> echo "${CURDIR}/target/packages/p/pandoc3/pandoc3-3.5-3-1-x86_64.eopkg"
...
target/packages/b/bb/bb-1.12.194-2-1-x86_64.eopkg: target/Taskfile.yml
> mkdir -p target/packages/b/bb
> cp -f src/bb.yml target/packages/b/bb/package.yml
> (cd target/packages/b/bb && go-task)

bb: target/packages/b/bb/bb-1.12.194-2-1-x86_64.eopkg ## build bb, print path to new eopkg file
> echo "${CURDIR}/target/packages/b/bb/bb-1.12.194-2-1-x86_64.eopkg"
